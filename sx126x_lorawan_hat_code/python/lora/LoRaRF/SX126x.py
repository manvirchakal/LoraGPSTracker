...
try:
    ...
    try:
        ...
        # Define constants for lgpio that are in RPi.GPIO
        if not hasattr(gpio, 'HIGH'):
            gpio.HIGH = 1
        if not hasattr(gpio, 'LOW'):
            gpio.LOW = 0
        if not hasattr(gpio, 'RISING'):
            gpio.RISING = lgpio.RISING_EDGE
        if not hasattr(gpio, 'FALLING'):
            gpio.FALLING = lgpio.FALLING_EDGE
    except ImportError:
        raise ImportError("Neither RPi.GPIO nor lgpio is available. Please install one of these packages.")
...
class SX126x(BaseLoRa) :
    ...
    def _gpio_write(self, pin, value):
        """Write to GPIO pin with compatibility for both RPi.GPIO and lgpio."""
        global USE_LGPIO, _lgpio_handle
        
        if USE_LGPIO:
            # lgpio has different API
            if value == gpio.HIGH:
                gpio.gpio_write(_lgpio_handle, pin, 1)
            else:
                gpio.gpio_write(_lgpio_handle, pin, 0)
        else:
            # RPi.GPIO
    ...
    def _gpio_read(self, pin):
        """Read from GPIO pin with compatibility for both RPi.GPIO and lgpio."""
        global USE_LGPIO, _lgpio_handle
        
        if USE_LGPIO:
            # lgpio has different API
            return gpio.gpio_read(_lgpio_handle, pin)
        else:
            # RPi.GPIO
...
### COMMON OPERATIONAL METHODS ###
    ...
    def end(self) :
        ...
        if USE_LGPIO:
            ...
            if _lgpio_handle is not None:
                gpio.gpiochip_close(_lgpio_handle)
        else:
            # RPi.GPIO cleanup
            gpio.cleanup()
    ...
    def wake(self) :
        ...
        if (self._wake != -1) :
            ...
            if USE_LGPIO:
                # lgpio version
                gpio.gpio_claim_output(_lgpio_handle, self._wake)
                gpio.gpio_write(_lgpio_handle, self._wake, 0)  # LOW
            else:
                # RPi.GPIO version
    ...
    def busyCheck(self, timeout: int = _busyTimeout) :
        ...
        t = time.time()
        while self._gpio_read(self._busy) == gpio.HIGH :
...
### HARDWARE CONFIGURATION METHODS ###
    ...
    def setPins(self, reset: int, busy: int, irq: int = -1, txen: int = -1, rxen: int = -1, wake: int = -1) :
        ...
        if USE_LGPIO:
            ...
            gpio.gpio_claim_output(_lgpio_handle, reset)
            gpio.gpio_claim_input(_lgpio_handle, busy)
            gpio.gpio_claim_output(_lgpio_handle, self._cs_define)
            
            if irq != -1:
                gpio.gpio_claim_input(_lgpio_handle, irq)
            if txen != -1:
                gpio.gpio_claim_output(_lgpio_handle, txen)
            # if rxen != -1:
            #     gpio.gpio_claim_output(_lgpio_handle, rxen)
        else:
            # RPi.GPIO
...
### TRANSMIT RELATED METHODS ###
    ...
    def endPacket(self, timeout: int = TX_SINGLE) -> bool :
        ...
        if self._irq != -1 :
            ...
            if USE_LGPIO:
                # For lgpio we need a different approach for interrupts
                gpio.gpio_claim_alert(_lgpio_handle, self._irq, lgpio.RISING_EDGE)
                gpio.callback(_lgpio_handle, self._irq, lgpio.RISING_EDGE, self._interruptTx)
            else:
                # RPi.GPIO approach
...
### RECEIVE RELATED METHODS ###
    ...
    def request(self, timeout: int = RX_SINGLE) -> bool :
        ...
        if self._txen != -1 :
            self._txState = self._gpio_read(self._txen)
            self._gpio_write(self._txen, gpio.HIGH)
        ...
        if self._irq != -1 :
            ...
            if USE_LGPIO:
                # For lgpio we need a different approach for interrupts
                # This is a basic setup - you may need to customize based on your needs
                if timeout == self.RX_CONTINUOUS:
                    gpio.gpio_claim_alert(_lgpio_handle, self._irq, lgpio.RISING_EDGE)
                    # You would need to handle callbacks differently with lgpio
                    # This is a simplified example
                    gpio.callback(_lgpio_handle, self._irq, lgpio.RISING_EDGE, self._interruptRxContinuous)
                else:
                    gpio.gpio_claim_alert(_lgpio_handle, self._irq, lgpio.RISING_EDGE)
                    gpio.callback(_lgpio_handle, self._irq, lgpio.RISING_EDGE, self._interruptRx)
            else:
                # RPi.GPIO approach
                gpio.remove_event_detect(self._irq)
                if timeout == self.RX_CONTINUOUS:
...
### INTERRUPT HANDLER METHODS ###
    ...
    def _interruptTx(self, channel) :
        ...
        if self._txen != -1 :
            self._gpio_write(self._txen, self._txState)
        # store IRQ status
...
### SX126X API: UTILITIES ###
    ...
    def _writeBytes(self, opCode: int, data: tuple, nBytes: int) :
        ...
        self._gpio_write(self._cs_define, gpio.LOW)
        buf = [opCode]
        for i in range(nBytes) : buf.append(data[i])
        spi.xfer2(buf)
        self._gpio_write(self._cs_define, gpio.HIGH)
